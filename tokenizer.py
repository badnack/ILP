...
class Tokenizer:
  ...
  def next_token(self):
    ...
    if self.pos >= len(self.input_string):
      ...
      self.pos += 1
    elif len(self.input_string) > self.pos + 1 and self.input_string[self.pos:self.pos+2] == "<=":
      self.current_token = ("<=", "hypothetical")
      self.pos += 2
    elif self.input_string[self.pos: self.pos+2] == ":-":
      self.pos += 2
      self.current_token = (":-", "declaration")
    elif self.input_string[self.pos] in ".,":
